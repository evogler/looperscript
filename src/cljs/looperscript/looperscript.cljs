...
(defn next-note-fn [part start-time]
  (let [elements [:time :sound :volume :filter :pan :rate]
        _ (doseq [e elements])
  ...
  (let [sound (:sound n)
        ...
        node (if (number? sound)
               (if filter
                 ...
                 (audio/play-tone          sound start-time dur vol pan))
               (audio/play-sound sound start-time vol rate))]
    (add-note-to-sounding-notes n node)))
