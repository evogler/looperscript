...
;;         poses (zipmap elements (repeat (atom 0)))
;;         queues (zipmap elements (repeat (atom [])))
...
;;                                      (if-let [[r] @q]
;;                                        (do (swap! q rest) r)
;;                                        (let [x (-> (get @element-vecs e)
;;                                                    (nth @pos))]
;;                                          ;; increment pos
;;                                          (swap! pos #(mod (inc %) (get @lengths e)))
;;                                          ;; queue and return first or just return
;;                                          (if (coll? x)
;;                                            (let [r (first x)]
;;                                              (reset! q (rest x)) r)
;;                                            x))))
